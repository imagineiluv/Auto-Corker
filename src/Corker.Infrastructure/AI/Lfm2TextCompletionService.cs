...
public class Lfm2TextCompletionService : ILLMService, ILLMStatusProvider, IDisposable
{
    ...
    public async Task InitializeAsync()
    {
        ...
        try
        {
            ...
            {
                ContextSize = (uint)settings.ContextWindow, // Use configured context window
    ...
    public async Task<string> GenerateTextAsync(string prompt, CancellationToken cancellationToken = default)
    {
        ...
        var inferenceParams = new InferenceParams() { MaxTokens = 2048, AntiPrompts = new List<string> { "User:", "\nUser:" } };
