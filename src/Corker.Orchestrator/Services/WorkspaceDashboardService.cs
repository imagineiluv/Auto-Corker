...
public class WorkspaceDashboardService
{
    ...
    public async Task<IReadOnlyList<IssueItem>> CreateTaskFromGithubIssueAsync(int issueNumber)
    {
        ...
        lock (_listLock)
        {
            issue = _issues.FirstOrDefault(candidate => candidate.Number == issueNumber);
        }

        if (issue is null)
        {
            lock (_listLock) return _issues.ToList();
        }
    ...
    public async Task<IReadOnlyList<IssueItem>> CreateTaskFromGitLabIssueAsync(int issueNumber)
    {
        ...
        lock (_listLock)
        {
            issue = _gitLabIssues.FirstOrDefault(candidate => candidate.Number == issueNumber);
        }

        if (issue is null)
        {
            lock (_listLock) return _gitLabIssues.ToList();
        }
    ...
    {
        await EnsureSeededAsync();
        lock (_listLock) return _pullRequests.ToList();
    }
    ...
    public async Task<IReadOnlyList<PullRequestItem>> CreateTaskFromPullRequestAsync(int pullRequestNumber)
    {
        ...
        lock (_listLock) pullRequest = _pullRequests.FirstOrDefault(pr => pr.Number == pullRequestNumber);
    ...
    {
        await EnsureSeededAsync();
        lock (_listLock) return _mergeRequests.ToList();
    }
    ...
    public async Task<IReadOnlyList<MergeRequestItem>> CreateTaskFromMergeRequestAsync(int mergeRequestNumber)
    {
        ...
        lock (_listLock) mergeRequest = _mergeRequests.FirstOrDefault(mr => mr.Number == mergeRequestNumber);
    ...
    {
        await EnsureSeededAsync();
        lock (_listLock) return _agentTools.ToList();
    }
    ...
    {
        await EnsureSeededAsync();
        lock (_listLock) return _worktrees.ToList();
    }
    ...
    {
        await EnsureSeededAsync();
        lock (_listLock) return _terminals.ToList();
    }
    ...
    {
        await EnsureSeededAsync();
        lock (_listLock) return _changelog.ToList();
    }
    ...
    {
        await EnsureSeededAsync();
        lock (_listLock) return _insightMetrics.ToList();
    }
    ...
    {
        await EnsureSeededAsync();
        lock (_listLock) return _roadmapGroups.ToList();
    }

    public async Task<IReadOnlyList<RoadmapPhase>> GetRoadmapByPhaseAsync()
    {
        await EnsureSeededAsync();
        lock (_listLock) return _roadmapPhases.ToList();
    }
    ...
    {
        await EnsureSeededAsync();
        lock (_listLock) return new MemoryOverview(_memoryItems.Count, _memoryItems.ToList());
    }
    ...
    public async Task<MemoryOverview> ManageMemoryCardsAsync()
    {
        ...
        return await GetMemoryOverviewAsync();
    }
